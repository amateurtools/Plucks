...
juce::AudioProcessorValueTreeState::ParameterLayout PlucksAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterBool>(
        juce::ParameterID { "INTERPOLATION", 1 },
        "Interpolation",
        false));
...
void PlucksAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // Increment the block counter and wrap by modulus
    const int blocksPerParamUpdate = 4; // update params every 4 blocks
    blockCounter = (blockCounter + 1) % blocksPerParamUpdate;
    if (blockCounter == 0)
    {
        ...
        for (int i = 0; i < synth.getNumVoices(); ++i)
        {
            ...
            {
                if (stereoEnabled != oldStereoEnabled){
                    pluckVoice->setStereoEnabled(stereoEnabled);
                    oldStereoEnabled = stereoEnabled;
                }
                if (newFineTuneCents != oldFineTuneCents){
                    pluckVoice->setFineTuneCents(newFineTuneCents);
                    oldFineTuneCents = newFineTuneCents;
                }
