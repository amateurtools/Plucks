...
public:
    ...
    void startNote(int midiNoteNumber, float velocity, juce::SynthesiserSound*, int) override
    {
        ...
        smoothedDelayLength.reset(currentSampleRate, 0.01);
    ...
    void renderNextBlock(juce::AudioBuffer<float>& outputBuffer, int startSample, int numSamples) override
    {
        ...
        // float feedbackGain = std::pow(targetAmplitude, 1.0f / (1.7f * decayTimeSeconds * cyclesPerSecond)); // forgot why the 1.7f
        float feedbackGain = std::pow(targetAmplitude, 1.0f / (decayTimeSeconds * cyclesPerSecond));
        ...
        float currentDelayValue = smoothedDelayLength.getCurrentValue();
        ...
        leftDelayLine.setDelay(currentDelayValue);
        rightDelayLine.setDelay(currentDelayValue);
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            if (activeSampleCounter < currentDelayValue && activeSampleCounter <= currentExciterSize)
            {
                addL += exciterLeft[activeSampleCounter] * currentVelocity;
                addR += exciterRight[activeSampleCounter] * currentVelocity;
            }

            if (reExciterIndex >= 0 && reExciterIndex < baseExactDelayInt && reExciterIndex < currentExciterSize)
            {
    ...
    void generateExciter(float currentVelocity, std::vector<float>& exciterL, std::vector<float>& exciterR)
    {
        ...
        for (int i = 0; i < safeDelayInt; ++i)
        {
            ...
            if (stereoEnabled)
            {
                ...
                exciterR[i] = juce::jmap(currentColor, squareSample, noiseSampleR); 
            }
            else
            {
                exciterL[i] = excitationL;
        ...
        currentExciterSize = exciterL.size(); // used in renderNextBlock
